...
internal class MappingGenerateState
{
	...
	{
		LastGeneratedPosition = new(
			line: LastGeneratedPosition.Line + 1,
			column: 0);
	}
	...
	{
		LastGeneratedPosition = new(
			line: LastGeneratedPosition.Line,
			column: zeroBasedColumnNumber);
	}
}
...
public class SourceMapGenerator
{
	...
	{
		if (state.LastGeneratedPosition.Line > entry.SourceMapPosition.Line)
		{
			throw new InvalidOperationException($"Invalid sourmap detected. Please check the line {entry.SourceMapPosition.Line}");
		}
			
		// Each line of generated code is separated using semicolons
		while (entry.SourceMapPosition.Line != state.LastGeneratedPosition.Line)
		{
		...
		Base64VlqEncoder.Encode(output, entry.SourceMapPosition.Column - state.LastGeneratedPosition.Column);
		state.UpdateLastGeneratedPositionColumn(entry.SourceMapPosition.Column);
		...
		if (entry.OriginalFileName != null)
		{
			...
			Base64VlqEncoder.Encode(output, entry.SourcePosition.Line - state.LastOriginalPosition.Line);
			Base64VlqEncoder.Encode(output, entry.SourcePosition.Column - state.LastOriginalPosition.Column);

			state.LastOriginalPosition = new(
				line: entry.SourcePosition.Line,
				column: entry.SourcePosition.Column);
