...
{
	private readonly Regex _lineNumberRegex = new(@"([^@(\s]*\.js)[^/]*:([0-9]+):([0-9]+)[^/]*$", RegexOptions.Compiled);
	...
	protected internal virtual StackFrame TryParseSingleStackFrame(string frame)
	{
		...
		if (!lineNumberMatch.Success) return null;
		...
		if (lineNumberMatch.Success)
		{
			...
			result.SourcePosition = new(
				...
				line: int.Parse(lineNumberMatch.Groups[2].Value, CultureInfo.InvariantCulture) - 1,
				column: int.Parse(lineNumberMatch.Groups[3].Value, CultureInfo.InvariantCulture) -1);
		}
