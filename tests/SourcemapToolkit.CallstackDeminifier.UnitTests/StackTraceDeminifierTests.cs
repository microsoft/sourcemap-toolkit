...
public class StackTraceDeminifierTests
{
	...
	private static void ValidateDeminifyStackTraceResults(DeminifyStackTraceResult results)
	{
		...
		Assert.Equal("level3", results.DeminifiedStackFrameResults[0].StackFrame.MethodName);
		...
		Assert.Equal("window.onload", results.DeminifiedStackFrameResults[5].StackFrame.MethodName);
	}
