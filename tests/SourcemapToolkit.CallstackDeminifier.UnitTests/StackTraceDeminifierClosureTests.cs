...
public class StackTraceDeminifierClosureTests
{
	...
	private StackTraceDeminifier GetStackTraceDeminifierWithDependencies()
	{
		...
		sourceMapProvider.Setup(x => x.GetSourceMapContentsForCallstackUrl("http://localhost:11323/closurecrashcauser.minified.js"))
	...
	private void ValidateDeminifyStackTraceResults(DeminifyStackTraceResult results)
	{
		...
		Assert.Equal(DeminificationError.None, results.DeminifiedStackFrameResults[0].Error);
		Assert.Equal("mynamespace.objectWithMethods.propertyMethodLevel2", results.DeminifiedStackFrameResults[0].StackFrame.MethodName);
		Assert.Equal("mynamespace.objectWithMethods.prototypeMethodLevel1", results.DeminifiedStackFrameResults[1].StackFrame.MethodName);
		Assert.Equal("GlobalFunction", results.DeminifiedStackFrameResults[2].StackFrame.MethodName);
		Assert.Equal("window.onload", results.DeminifiedStackFrameResults[3].StackFrame.MethodName);
	}
